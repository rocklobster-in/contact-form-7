...
const update = (form) => {
  ...
  form.querySelectorAll(".tag").forEach((tag) => {
    const basetype = form.querySelector('[name="tagtype"]')?.value || tag.name;
  ...
  form.querySelectorAll("span.mail-tag").forEach((span) => {
    span.innerText = `[${name}]`;
  });

  form.querySelectorAll("input.mail-tag").forEach((input) => {
    input.value = `[${name}]`;
  });
};
...
const compose = (basetype, form) => {
  ...
  const type =
    basetype + (form.querySelector('[name="required"]:checked') ? "*" : "");

  const name = form.querySelector('[name="name"]')?.value ?? "";

  const options = [];

  scope.querySelectorAll(".option").forEach((input) => {
    if ("checkbox" === input.type) {
      ...
      if (input.classList.contains("filetype")) {
        options.push(`${input.name}:${input.value.split(/[,|\s]+/).join("|")}`);
