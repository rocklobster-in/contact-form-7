const init = (form) => {
  ...
  form.querySelectorAll(".control-box").forEach((box) => {
...
const update = (form) => {
  ...
  if (composed) {
    form.querySelectorAll('[data-tag-part="tag"]').forEach((tag) => {
      tag.value = composed;
    });

    form.querySelectorAll('[data-taggen="insert-tag"]').forEach((tag) => {
      tag.disabled = false;
    });

    form.querySelectorAll('[data-tag-part="mail-tag"]').forEach((tag) => {
...
const compose = (scope) => {
  const basetype = scope
    .querySelector('[data-tag-part="basetype"]')
    ?.value.trim();
  ...
  const typeSuffixField = scope.querySelector('[data-tag-part="type-suffix"]');
  ...
  scope.querySelectorAll('[data-tag-part="option"]').forEach((field) => {
  ...
  const values =
    scope
      .querySelector('[data-tag-part="value"]')
      ?.value.split("\n")
      .map((line) => line.trim())
      .filter((line) => "" !== line)
      .map((line) => `"${line.replace(/["]/g, "&quot;")}"`) ?? [];

  const composed = [type, name, options.join(" "), values.join(" ")]
    .map((item) => item.trim())
    .filter((item) => "" !== item);

  const content = scope
    .querySelector('[data-tag-part="content"]')
    ?.value.trim();
