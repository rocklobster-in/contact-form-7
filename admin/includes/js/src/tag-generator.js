...
const init = () => {
  document.querySelectorAll('[data-taggen="open-dialog"]').forEach((button) => {
  ...
  document.querySelectorAll("dialog.tag-generator-dialog").forEach((dialog) => {
    dialog
      .querySelectorAll('[data-taggen="close-dialog"]')
      .forEach((button) => {
        button.addEventListener("click", (event) => dialog.close(""));
      });

    dialog
      .querySelectorAll('[data-taggen="insert-tag"], .insert-tag')
      .forEach((button) => {
        button.addEventListener("click", (event) => {
