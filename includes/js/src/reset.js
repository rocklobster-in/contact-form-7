...
export const resetCaptcha = (form, refill) => {
  for (const name in refill) {
    ...
    form
      .querySelectorAll(`img.wpcf7-captcha-${name.replaceAll(":", "")}`)
      .forEach((img) => {
    ...
    if (match) {
      form
        .querySelectorAll(`input[name="_wpcf7_captcha_challenge_${name}"]`)
        .forEach((input) => {
          input.value = match[1];
        });
    }
...
export const resetQuiz = (form, refill) => {
  for (const name in refill) {
    ...
    form
      .querySelectorAll(`.wpcf7-form-control-wrap[data-name="${name}"]`)
      .forEach((wrap) => {
        wrap.querySelector(`input[name="${name}"]`).value = "";
        wrap.querySelector(".wpcf7-quiz-label").textContent = question;

        wrap.querySelector(`input[name="_wpcf7_quiz_answer_${name}"]`).value =
          hashedAnswer;
      });
  }
};
