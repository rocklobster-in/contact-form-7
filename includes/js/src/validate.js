...
export default async function validate(form, options = {}) {
  const { target, scope = form, ...remainingOptions } = options;
...
export const setValidationError = (form, fieldName, message, options) => {
  const { scope = form, ...remainingOptions } = options ?? {};

  const errorId = `${form.wpcf7?.unitTag}-ve-${fieldName}`.replaceAll(
    /[^0-9a-z_-]+/gi,
    "",
  );
  ...
  const setVisualValidationError = () => {
    scope
      .querySelectorAll(`.wpcf7-form-control-wrap[data-name="${fieldName}"]`)
      .forEach((wrap) => {
...
export const removeValidationError = (form, fieldName) => {
  ...
  form.wpcf7.parent
    .querySelector(`.screen-reader-response ul li#${errorId}`)
    ?.remove();
...
export const updateReflection = (form, field, validInputs) => {
  form
    .querySelectorAll(`[data-reflection-of="${field}"]`)
    .forEach((reflection) => {
      if ("output" === reflection.tagName.toLowerCase()) {
        ...
        reflection.querySelectorAll("output").forEach((output) => {
