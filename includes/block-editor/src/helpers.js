...
export const createShortcode = (attributes) => {
  ...
  if (attributes.hash) {
    shortcode = shortcode.replace(/\]$/, ` id="${attributes.hash}"]`);
  } else if (attributes.id) {
    shortcode = shortcode.replace(/\]$/, ` id="${attributes.id}"]`);
  }

  if (attributes.title) {
    shortcode = shortcode.replace(/\]$/, ` title="${attributes.title}"]`);
  }

  if (attributes.htmlId) {
    shortcode = shortcode.replace(/\]$/, ` html_id="${attributes.htmlId}"]`);
  }
  ...
  if ("raw_form" === attributes.output) {
    shortcode = shortcode.replace(/\]$/, ` output="${attributes.output}"]`);
  }
